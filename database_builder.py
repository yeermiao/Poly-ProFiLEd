...
DATA_BASE_FILE_PATH = 'F:\\Users\\yeerm\\Desktop\\Masterarbeit\\Messdaten'
...
def extremum_derivative(t, a, b, c, d):
    ...
    derivative = -a * b * np.exp(-b * t) * np.cos(c * t + d)+ a * c * np.exp(-b * t) * np.sin(c * t + d)
    return derivative
...
def calculate_r_squared(measurement: pd.DataFrame, col_name: str, params: list[float]):
